...
3. Find the `AI_MODELS` dictionary and set:
   ```python
...
`<CodeProject id="deepfake-detector">````markdown
# Detector Conan - Deepfake Detection Platform
...
\`\`\`bash
git clone https://github.com/tahangz/Detector_Conan_for_deepfakes
cd Detector_Conan_for_deepfakes
\`\`\`
...
Replace the MongoDB URI and JWT secret with your own values. For example:

\`\`\`
MONGODB_URI=mongodb+srv://tahanaguez:Taha20188555013@cluster0.1u3vcmn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0
JWT_SECRET=H2j#X9s!pQrT7@vLmZcD4^wF8&bKoN5y
PORT=3000
FRONTEND_URL=http://localhost:5000
\`\`\`
...
2. For Windows users, update the start script in `package.json`:
...
Open `DJ_API/deepfake_api/settings.py` and modify the AI_MODELS configuration:
...
1. Check if the model files exist in the correct location
2. Verify that TensorFlow and PyTorch are properly installed
3. Look at the Django logs for specific error messages



3. **CORS Issues**:

1. Ensure the frontend URL is correctly set in Django's CORS settings
2. Check that the backend URL is correctly set in the frontend's axios configuration



4. **File Upload Problems**:

1. Verify that the `uploads` directory exists in the backend
